...
export class ImageService {
  ...
  private isElectronics(name: string): boolean {
    const electronicsKeywords = ['phone', 'laptop', 'computer', 'tablet', 'headphone', 'speaker', 'camera', 'tv', 'monitor', 'keyboard', 'mouse', 'electronic', 'digital', 'smart', 'wireless', 'bluetooth', 'usb', 'charger', 'battery'];
    return electronicsKeywords.some(keyword => name.includes(keyword));
  }

  private isHomeAppliance(name: string): boolean {
    const homeKeywords = ['mixer', 'grinder', 'blender', 'refrigerator', 'fridge', 'washing', 'machine', 'microwave', 'oven', 'stove', 'fan', 'ac', 'air', 'conditioner', 'iron', 'vacuum', 'cleaner', 'appliance', 'kitchen', 'home'];
  ...
  private isClothing(name: string): boolean {
    const clothingKeywords = ['shirt', 't-shirt', 'tshirt', 'pant', 'jeans', 'dress', 'skirt', 'jacket', 'coat', 'sweater', 'hoodie', 'shorts', 'underwear', 'sock', 'shoe', 'boot', 'sandal', 'hat', 'cap', 'belt', 'bag', 'purse', 'wallet', 'clothing', 'apparel', 'fashion', 'wear'];
    return clothingKeywords.some(keyword => name.includes(keyword));
  }

  private isBook(name: string): boolean {
    const bookKeywords = ['book', 'novel', 'magazine', 'newspaper', 'notebook', 'pen', 'pencil', 'paper', 'stationery', 'dictionary', 'encyclopedia', 'textbook', 'guide', 'manual', 'literature', 'reading', 'study', 'education'];
